...
// Newsletter subscription endpoint for Pure Nail Bar. Validates the submitted
...
export async function POST(request: Request) {
  ...
  if (!supabase) {
    console.warn("[newsletter] Supabase not configured — accepting subscription in development:", email);
  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.json(
        { success: false, error: "Unable to subscribe at this time. Please try again shortly." },
        { status: 503 },
      );
    }
    ...
    console.error("[newsletter] unexpected error:", err instanceof Error ? err.message : err);
    return NextResponse.json(
      { success: false, error: "An unexpected error occurred. Please try again shortly." },
      { status: 500 },
    );
  }
}
