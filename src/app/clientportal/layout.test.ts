...
describe("clientportal layout generateMetadata — per-tenant favicon", () => {
  ...
  it("keeps the client-portal title and noindex robots", async () => {
    faviconValue = "https://cdn.example/x.png";
    const meta = await generateMetadata();
    expect(meta.title).toBe("Client Portal");
    expect(meta.robots).toEqual({ index: false, follow: false });
  });

  // Must render dynamically — a static prerender bakes the build-time tenant
  // favicon into the HTML (same rationale as the /checkin layout).
