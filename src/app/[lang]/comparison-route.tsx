...
// Shared comparison-page route. The FR (/comparaisons) and EN (/comparisons)
// folders differ by exactly one fact — the locale they serve — so the whole
// body (metadata + render) lives here once and each folder's page.tsx is a
// thin shell binding its `only` locale. comparisonBySlug → notFound() handles
// unknown or wrong-locale slugs: a slug from the other locale fails the lookup
// under `only` and 404s. No generateStaticParams — slugs resolve at request
// time against the runtime tenant (parent [lang] layout is force-dynamic).

type Params = { params: Promise<{ lang: string; slug: string }> };

export function makeComparisonRoute(only: Locale) {
