...
// /sainte-foy serves both FR and EN (same slug — proper noun, no localization needed).
...
export async function generateMetadata({ params }: LangParams): Promise<Metadata> {
  ...
  return page.metadata("/sainte-foy", {
    title: seo.pages.nearMe.metaTitle,
    description: seo.pages.nearMe.metaDescription,
    routeByLocale: { fr: "/sainte-foy", en: "/sainte-foy" },
  });
}

export default async function SainteFoyPage({ params }: LangParams) {
  ...
  return (
    <>
      {/* Breadcrumb JSON-LD: Home → Locations → Sainte-Foy */}
      <JsonLd
        data={page.breadcrumb([
          { name: dict.nav.home, route: "" },
          { name: dict.locations.heading, route: "/locations" },
          { name: seo.pages.nearMe.boroughName || "Sainte-Foy", route: "/sainte-foy" },
        ])}
      />
