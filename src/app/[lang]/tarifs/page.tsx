...
// Wrong-locale guard: /tarifs is FR-only; non-fr lang → 404.

export async function generateMetadata({ params }: LangParams): Promise<Metadata> {
  const { lang } = await params;
  if (!isLocale(lang)) return {};
  // Wrong-locale guard: /tarifs is the French pricing slug only.
  if (lang !== "fr") return {};
...
export default async function TarifsPage({ params }: LangParams) {
  const { lang } = await params;
  if (!isLocale(lang)) notFound();
  // Wrong-locale guard: /tarifs is FR-only (Pitfall 1).
  if (lang !== "fr") notFound();
